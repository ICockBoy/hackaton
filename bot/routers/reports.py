...
async def button_reports(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == reports_buttons.choose1:
        data["problem_type"] = reports_buttons.choose1.text
    if callback.data == reports_buttons.choose2:
...
async def report_accept(callback: CallbackQuery, state: FSMContext):
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="Проблема решена", callback_data=f"problem_accept/{callback.message.chat.id}"))
    kb.row(InlineKeyboardButton(text="Написать пользователю", url=f"https://t.me/{callback.message.from_user.username}"))
    house = dbase.houses.find_user_house(callback.message.chat.id)
    bot.send_message(house.moderate_group_id,
                     callback.message.text,
                     message_thread_id=house.moderate_topic_reports,
                     reply_markup=kb.as_markup())
    await callback.message.edit_text(reports_texts.problem_to_moderate, reply_markup=kb.as_markup())
    await state.set_state(start_states.none)


@router.callback_query(Text(reports_buttons.report_accept.problem_accept))
async def problem_accept(callback: CallbackQuery, state: FSMContext):
    user_id = callback.data.split("/")[1]
    bot.send_message(user_id, reports_texts.problem_is_not_problem + "\n\n" + callback.message.text)
